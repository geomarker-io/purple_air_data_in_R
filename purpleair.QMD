...
<!--# This section here will be simplified with a source file 'new_pas.R' that will do most of this process automatically. -->
...
4.  In the R script that you want to read in the API Key, use the `source()` function and then reference the R file (in this case, `global_vars.R`) that you saved in the previous step.
    ...
    ::: callout-caution
    If you save your `global_vars.R` file in a different file location on your computer than the R script that you are using to pull the API Key from, you must specify the full file path in quotes inside the `source()` function. You can use the `here` package to specify the file location where `global_vars.R` is saved. See @sec-saveload for more details.
    :::
...
<!--# This section will be eliminated. A source file 'new_pas.R' will do this process automatically. -->
...
<!--# This section will be rewritten and simplified. A source file 'new_pas.R' will do this process automatically. -->
