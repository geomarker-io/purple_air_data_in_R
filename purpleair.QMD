...
Download the `purpleair.QMD` and the `purpleair.bib` file from our [GitHub page](https://github.com/geomarker-io/purple_air_data_in_R). Save both files in the same file location on your computer. Click on the `purpleair.QMD` file to open it.

This file is called a Quarto file, identified with the file extension `.QMD`. This kind of document will allow the user to display R code and output while also explaining what the R code does. R code is displayed in R code chunks like the following example below. To execute the entirety of an R code chunk, click on the green arrow button at the top right of the R code chunk.
...
If you run into problems running and compiling this document, a rendered HTML of the output is hosted online on our GitHub page, which you can access by clicking [here](https://geomarker.io/purple_air_data_in_R/). In addition to the Quarto document, we have pulled all the R code from purpleair.QMD and saved them in a regular R script file (identified with the file extension `.R`). This is handy if you are interested in playing around with the R code provided here or for learning more about how the functions work.
